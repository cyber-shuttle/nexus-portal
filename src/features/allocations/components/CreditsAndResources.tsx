...
export function CreditsAndResources({ allocationId }: { allocationId: string }) {
      ...
      {resources.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          {resources.map((resource, i) => {
            ...
            const allocated = allocation?.initial_su_amount ?? 0;
            const used = 0;
            return (
              <article key={resource.id} className="rounded-md border bg-card p-5">
                ...
                <div className="mt-4 space-y-2">
                  <div className="flex items-baseline justify-between text-xs text-muted-foreground">
                    <span>Used</span>
                    <span>
                      {formatSU(used)} / {formatSU(allocated)}
                    </span>
                  </div>
                  <UsageBar value={used} max={allocated} size="sm" />
                </div>
