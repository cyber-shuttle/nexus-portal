...
describe("amie api", () => {
  it("serializes listPackets params into the query string", async () => {
    ...
    expect(url).toContain("/api/v1/amie/packets");
  ...
  it("strips empty params from the query string", async () => {
  ...
  it("throws ApiError when the upstream returns a 404", async () => {
  ...
  it("listUnmapped goes to the unmapped endpoint", async () => {
    fetchMock.mockResolvedValueOnce(jsonResponse({ packets: [], total: 0, limit: 0, offset: 0 }));
    await listUnmapped({ limit: 25 });
    expect(fetchMock.mock.calls[0]?.[0]).toContain("/api/v1/amie/unmapped");
  });
  ...
  it("getPacketEvents parses an array of events", async () => {
    fetchMock.mockResolvedValueOnce(
      jsonResponse([
        {
          id: "evt-1",
