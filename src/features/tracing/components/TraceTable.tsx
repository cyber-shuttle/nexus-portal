...
function TraceRow({ trace, onView }: { trace: Trace; onView(id: string): void }) {
  ...
  // on the list shape, so we fall back to root event error text or root_name.
  ...
  // Pointer clicks on the row navigate; keyboard users get a dedicated
  // semantic button (covering the row via absolute fill) so we keep proper
  // focusable semantics without nesting a button inside another button.
  return (
    // biome-ignore lint/a11y/useKeyWithClickEvents: keyboard nav routed through the inner <button> (focus-anchored, sr-only); the row's mouse handler is a pointer convenience only
    <div
      data-testid={`trace-row-${trace.trace_id}`}
      data-tone={tone}
      onClick={(e) => {
        // Ignore clicks that originated inside an interactive child (the copy
        // button stops propagation already, but defensive: bail on any button).
    ...
    >
      {isErr && (
        <span
          ...
          className="absolute left-0 top-0 bottom-0 w-[3px] bg-[color:var(--nexus-red-500)]"
        />
      )}
      ...
      <button
        ...
        // Visually hidden focus target spanning the whole row; keyboard users
        // tab here and press Enter/Space to navigate.
