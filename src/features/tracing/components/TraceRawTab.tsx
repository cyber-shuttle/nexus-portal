...
// Lexes the stringified JSON once and tags keys/strings/booleans/numbers with
// the design-spec'd colors. Anything outside a token (whitespace, punctuation)
// is emitted verbatim — preserves indent.
  ...
  const onCopy = () => {
    void (async () => {
      try {
        ...
        // Best-effort copy; still surface the check so the user gets feedback.
      }
      setCopied(true);
      if (timerRef.current) clearTimeout(timerRef.current);
