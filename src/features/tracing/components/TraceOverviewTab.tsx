...
export function TraceOverviewTab({ trace, spans }: TraceOverviewTabProps) {
  ...
  // Show ROOT span entity refs only — the first kind=1 entry, falling back to
  // the first span when no root marker is present (orphaned/synthesized data).
  ...
  const facts: Array<{ label: string; value: React.ReactNode }> = [
    ...
    },
    {
      ...
      value: trace.ended_at ? (
        ...
        <span className="italic text-[color:var(--nexus-amber-700)]">still running</span>
      ),
    },
    {
      label: "Duration",
