...
// API status codes: ok=0, error=1, cancelled=2, orphaned=3. `in-progress` is a
// UI-only filter (ended_at == null); never sent to the wire.
...
export function statusFiltersToApi(status: StatusFilter[]): {
  ...
  // When ONLY in-progress is selected we omit status from the wire (so the
