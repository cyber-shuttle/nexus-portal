...
import { AlertTriangle, ArrowDown, ArrowRight, ArrowUp, ChevronsDownUp, ChevronsUpDown } from "lucide-react";
...
export function TraceTreeTab({ trace, spans, onSwitchToTab }: TraceTreeTabProps) {
  ...
  // Re-seed when the trace changes — switching traces shouldn't carry over
  // a stale expanded-set keyed against the previous span IDs.
  const traceIdRef = React.useRef(trace.trace_id);
  if (traceIdRef.current !== trace.trace_id) {
    traceIdRef.current = trace.trace_id;
    // intentional sync state reset on prop change — guarded by ref so no loop
  ...
  const writeSelected = React.useCallback(
    (spanId: string) => {
  ...
  // Order error leaves by their visible-row index (using the unfiltered flat
  // list so `n`/`p` still works while in errors-only mode), so navigation
  // matches what the eye scans top-to-bottom.
  const errorLeavesInOrder = React.useMemo(() => {
    const all = flattenTree(roots, expanded, false, pathSet);
    return all.filter((v) => errorLeafSet.has(v.node.span.span_id)).map(
      (v) => v.node.span.span_id,
    );
  ...
  React.useEffect(() => {
    ...
    // jsdom lacks ResizeObserver; window resize alone is enough in tests.
  ...
  const scrollRowIntoView = React.useCallback((spanId: string) => {
    ...
    // Manual scroll math — element.scrollIntoView shifts the surrounding
    // drawer and breaks the rail's measured geometry.
  ...
  const gotoError = React.useCallback(
    (delta: number, cursorOverride?: number) => {
      ...
      const next =
        (base + delta + errorLeavesInOrder.length) % errorLeavesInOrder.length;
      setErrorCursor(next);
      const id = errorLeavesInOrder[next];
      if (!id) return;
      selectSpan(id);
      // Defer the scroll until after the row ref re-registers post-render
      // (selected styling can change row height by a hair).
  ...
  // Status summary — green/all-ok vs amber/running vs filtered count.
    ...
    <div className="flex h-full flex-col" data-testid="trace-tree-tab">
      ...
      <div className="flex min-h-0 flex-1 gap-6">
        <div
          ...
          // biome-ignore lint/a11y/noNoninteractiveTabindex: role="tree" is interactive per ARIA — must be focusable for keyboard nav.
          tabIndex={0}
        ...
        >
          ...
          {visible.length === 0 ? (
            ...
            visible.map((row) => {
              ...
              const hidden =
                row.hasChildren && !isExpanded && subtreeHasError(row.node);
              return (
                <TraceTreeRow
                  key={span.span_id}
                  row={row}
