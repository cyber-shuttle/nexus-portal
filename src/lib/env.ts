...
export const serverSchema = z
  ...
  // OIDC mode demands the four vars below — otherwise the runtime would
  // boot a misconfigured provider that silently fails at sign-in. Fail fast
  // at parse time so a bad VM .env never reaches users.
  .superRefine((env, ctx) => {
    if (env.PORTAL_AUTH_MODE !== "oidc") return;
